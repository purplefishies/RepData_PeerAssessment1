...
First it is important to replace the elements that are NA with 
at leat a best guess of each value.
ste

```{r,echo=TRUE}
replacements <- data %>% filter( !is.na(steps)) %>% group_by(interval) %>% summarize( ave_steps = floor(mean(steps)))
```
